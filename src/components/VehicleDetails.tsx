...
const VehicleDetail: React.FC = () => {
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden font-sans">
      <div className="absolute inset-0 w-full h-full z-0">
        {selectedVehicle && selectedVehicle.latitude && selectedVehicle.longitude ? (
          <MapContainer
            ...
            zoom={13}
            scrollWheelZoom
            className="h-full w-full z-0 animate-in fade-in zoom-in duration-700 ease-in-out"
          >
            <TileLayer
      ...
      {/* Sidebar Card - Posisikan kanan di desktop */}
        <div className="absolute z-10 md:top-[64px] md:right-0 md:bottom-0 md:w-[28rem] md:px-0 md:py-0 px-6 bottom-6 left-0 right-0 flex md:justify-end justify-center">
        <Card className="rounded-none md:rounded-l-2xl bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-xl border border-gray-200/50 dark:border-gray-700/50 w-full max-w-md sm:max-w-lg transition-all duration-500 ease-in-out hover:shadow-2xl animate-in md:slide-in-from-right-6 slide-in-from-bottom-6 overflow-y-auto max-h-[60vh] md:max-h-full">
          <CardHeader className="sticky top-0 bg-white/90 dark:bg-gray-900/90 backdrop-blur-lg z-10 px-6 py-4 border-b border-gray-200/50 dark:border-gray-700/50">
            <CardTitle className="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Detail Kendaraan</CardTitle>
          </CardHeader>
          <CardContent className="p-6 pt-4 space-y-4">
            ...
            {error && (
              <div className="text-red-500 text-center py-4 bg-red-50/80 dark:bg-red-900/50 rounded-lg shadow-sm">
                Error: {error}
              </div>
            )}
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-between mt-6">
                  <Button
                    ...
                    className="flex items-center gap-2 border-2 border-gray-300/50 dark:border-gray-600/50 text-gray-800 dark:text-gray-200 bg-gray-100/50 dark:bg-gray-800/50 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-medium transition-all duration-300 ease-in-out hover:shadow-md transform hover:scale-105 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
